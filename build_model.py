...
def bert_bilstm_crf(config_path, checkpoint_path, num_labels, lstm_units, drop_rate, leraning_rate):
    ...
    lstm = keras.layers.Bidirectional(
        keras.layers.LSTM(lstm_units,
                          ...
                          return_sequences=True))(x)  # [batch_size, seq_length, lstm_units * 2]

    x = keras.layers.concatenate([lstm, x],
                                 axis=-1)  # [batch_size, seq_length, lstm_units * 2 + 768]
...
if __name__ == '__main__':
    config_path = './bertbase/chinese_L-12_H-768_A-12/bert_config.json'
    checkpoint_path = './bertbase/chinese_L-12_H-768_A-12/bert_model.ckpt'
    num_labels = 21
    lstm_units = 128
    drop_rate = 0.1
    leraning_rate = 5e-5
    model, crf = bert_bilstm_crf(config_path,
                                 checkpoint_path,
                                 num_labels,
                                 lstm_units,
                                 drop_rate,
